...
export class PasswordRecoveryService {
  ...
  async resetPassword(email: string, otp: string, newPassword: string) {
    ...
    const updatedUser = await this.userRepo.updatePassword(user._id.toString(), newPassword);
    if (!updatedUser) {
      throw new CustomHttpException(500, "Password could not be updated");
    }

    const storedUser = await this.userRepo.findByIdWithPassword(user._id.toString());
    if (!storedUser || !(await bcrypt.compare(newPassword, storedUser.password))) {
      throw new CustomHttpException(500, "Password update could not be verified");
    }

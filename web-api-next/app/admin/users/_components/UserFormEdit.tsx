...
const fieldClass = "h-12 w-full border border-green-200 bg-white px-4 text-slate-950 placeholder:text-slate-500 outline-none transition-colors focus:border-green-700 rounded-md";
const labelClass = "mb-2 block text-xs font-bold uppercase tracking-[1.5px] text-slate-700";
    ...
    <div className="w-full max-w-md">
      <form onSubmit={onSubmit}>
        ...
        <div className="mb-5">
          ...
          <input type="file" onChange={handleImageChange} accept="image/*" className="text-sm text-slate-600" />
          {user?.profileImage && !imageFile && (
            <p className="mt-2 text-xs text-slate-500">Current image: {user.profileImage}</p>
          )}
        </div>
        ...
        <button
          type="submit"
          disabled={isPending}
          className="flex h-12 w-full items-center justify-center bg-green-950 text-xs font-bold uppercase tracking-[1.5px] text-white transition-opacity hover:bg-green-900 disabled:opacity-50 rounded-md"
        >

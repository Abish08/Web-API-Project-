...
export default function EditFoodPage() {
  ...
  const fetchFood = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        if (foodData.recipe) {
          setRecipe({
            ingredients: foodData.recipe.ingredients || [""],
            instructions: foodData.recipe.instructions || [""],
            prepTime: foodData.recipe.prepTime || 0,
            cookTime: foodData.recipe.cookTime || 0,
            servings: foodData.recipe.servings || 1,
            difficulty: (foodData.recipe.difficulty as "Easy" | "Medium" | "Hard") || "Medium",
          });
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // âœ… FIXED: Use credentials to send cookies automatically
      const response = await fetch(apiUrl(`/api/v1/foods/${params.id}`), {
        method: "PUT",
  ...
  if (loading) {
    return <div className="text-center py-12">Loading food...</div>;
  }
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-950">Edit Food</h1>
          <p className="text-sm text-slate-600">Update food information</p>
        </div>
        <Link
          href="/admin/foods"
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            >
              {submitLoading ? "Updating..." : "Update Food"}
            </button>
          </div>
        </form>
      </div>

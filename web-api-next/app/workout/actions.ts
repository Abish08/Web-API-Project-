...
export async function getWorkoutLogsAction(date: string) {
  try {
    ...
    const response = await fetch(`http://localhost:8089/api/v1/workout-logs?date=${date}`, {
...
export async function createWorkoutLogAction(logData: any) {
  try {
    const token = await getTokenCookie();
    if (!token) return { success: false, message: "Not authenticated" };

    const response = await fetch("http://localhost:8089/api/v1/workout-logs", {
      method: "POST",
...
export async function deleteWorkoutLogAction(id: string) {
  try {
    ...
    const response = await fetch(`http://localhost:8089/api/v1/workout-logs/${id}`, {
      method: "DELETE",
      headers: { "Authorization": `Bearer ${token}` },
    });
    return await response.json();
...
export async function searchWorkoutsAction(query: string) {
  try {
    const response = await fetch(`http://localhost:8089/api/v1/workouts/search?query=${query}`);
    return await response.json();
  } catch (error) {
